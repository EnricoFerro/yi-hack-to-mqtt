...
export class AnnounceService {
    ...
    initAnnunce(camera, icon, json_attributes_topic, state_topic, name, unique_id, value_template, unit_of_measurement, device_class?, qos?, payload_on?, payload_off?, off_delay?) {
        ...
        if (Configuration.cameras[camera].availability_topic === false) {
            announce.availability_topic = undefined;
            announce.payload_available = undefined;
        }
        return announce;
    }
    ...
    publishAnnounce(camera) {
        this.logger.debug(`Announce Camera ${camera}`)
        /**
         * Status.json path
         */
